...
export function generateStatSummary(user: User, quests: Quest[], logs: ActivityLog[]): string {
  ...
  // Compose summary. Remove the Hunter prefix for a more general description.
  let summary = `${user.name} — ${RANK_NAMES[user.rankTier]} (Level ${user.level}). `;
...
export function generateMonthlyReport(logs: ActivityLog[], quests: Quest[], month: number, year: number) {
  ...
  const completedQuestsCount = monthQuests.filter(q => q.status === 'completed').length;
  const failedQuestsCount = monthQuests.filter(q => q.status === 'failed').length;
  const totalQuests = monthQuests.length;
  const completionRateMonth = totalQuests > 0 ? Math.round((completedQuestsCount / totalQuests) * 100) : 0;

  // Activity breakdown
  const typeCountsMonth: Record<string, number> = {};
  monthLogs.forEach(l => {
    const label = ACTIVITY_TYPES[l.type]?.label || 'Other';
    typeCountsMonth[label] = (typeCountsMonth[label] || 0) + 1;
  });
  ...
  return {
    ...
    failedQuests: failedQuestsCount,
    totalQuests,
    completionRate: completionRateMonth,
    typeCounts: typeCountsMonth,
    dailyXP,
    demonCounts,
