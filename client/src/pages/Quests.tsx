 ...
 *
 * This version introduces custom quest durations, enlarges demon icons for better visibility and
 * improves accessibility of the create quest button. The frequency selector now includes a
 * "Custom" option that lets the user specify any number of days instead of being locked to 3.
 */
...
export default function Quests() {
  ...
  // Allow users to set a custom duration for quests when frequency is 'custom'
  const [qCustomDays, setQCustomDays] = useState(3);
  ...
  function handleCreateQuest() {
    ...
    if (qFrequency === 'daily') {
      ...
      // Custom frequency: add the specified number of days
      dueDate = new Date(now);
      dueDate.setDate(dueDate.getDate() + Math.max(1, qCustomDays));
      dueDate.setHours(23, 59, 59, 999);
    }
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {/* Custom now allows the user to specify the number of days separately */}
                      <option value="custom" className="bg-gray-900">Custom</option>
                    </select>
                  </div>
                </div>
                {/* Custom Days input displayed only when frequency is custom */}
                {qFrequency === 'custom' && (
                  <div>
                    <label className="text-[10px] text-gray-500 uppercase tracking-wider font-mono block mb-1">Custom Days</label>
                    <input
                      type="number"
                      value={qCustomDays}
                      onChange={e => setQCustomDays(Math.max(1, parseInt(e.target.value) || 1))}
                      className="w-full bg-white/[0.05] border border-white/10 rounded-sm px-3 py-2 text-sm text-white font-mono focus:border-cyan-500/50 focus:outline-none"
                      min={1}
                    />
                  </div>
      ...
      <SystemCard className={`${isCompleted ? 'opacity-60' : ''} ${isFailed ? 'opacity-50' : ''} ${isUrgent ? 'border-red-500/30' : ''}`}>
        <div className="flex items-start gap-3">
          <img
            ...
            className={`w-16 h-16 object-contain ${isCompleted ? 'grayscale' : ''} ${isFailed ? 'grayscale opacity-50' : ''}`}
          />
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`text-[8px] px-1.5 py-0.5 rounded-sm font-mono ${
                ...
                {quest.frequency === 'custom' ? `custom` : quest.frequency}
              </span>
            </div>
            {quest.description && (
